...
def poland_cases_by_date(day: int, month: int, year: int = 2020) -> int:
    ...
    """
    data = f"{month}/{day}/{str(year)[2:]}".lstrip("0").replace("/0", "/").replace(" 0", " ")
...
def top5_countries_by_date(day: int, month: int, year: int = 2020) -> List[str]:
    ...
    """
    data = f"{month}/{day}/{str(year)[2:]}".lstrip("0").replace("/0", "/").replace(" 0", " ")
    return confirmed_cases[["Country/Region", data]].groupby("Country/Region").sum().sort_values(by=data, ascending=False).head(5).index.values.tolist()
...
def no_new_cases_count(day: int, month: int, year: int = 2020) -> int:
    ...
    UWAGA: 
    należy zmienić swoją funkcję tak, żeby zwracała 
    ile jest krajów/regionów z nowymi przypadkami danego dnia.
    ...
    yesterdaystr = yesterday.strftime('%m/%d/%y').lstrip("0").replace(" 0", " ").replace("/0","/")
    return len(confirmed_cases.loc[confirmed_cases[d1]-confirmed_cases[yesterdaystr] != 0].index)
